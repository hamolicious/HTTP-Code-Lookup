...
def colour_print(color: str, *args, **kwargs) -> None:
...
def _search_by_text(text: str) -> None:
  ...
  for entry in subset:
    if text.lower() in entry.get('message', '').lower() or text.lower() in entry.get('desc', '').lower():
      search_results.append(entry)
...
def _display_data(search_term: str, results: list[dict]) -> None:
  colour_print('🔍', f'Results for "{search_term}"')
  colour_print(colorama.Fore.CYAN, '-' * 50)

  for entry in results:
    color = _get_color(entry.get("code"))
    colour_print(color + colorama.Style.BRIGHT, f'{entry.get("code")} - {entry.get("message")}')
    colour_print(color, entry.get("desc"))
    print()
...
def main(search_term: str, output_as_json: bool = False, no_pretty: bool = False, indent_size: int = 2, no_colour: bool = False) -> str | None:
  if no_colour:
  ...
  if output_as_json:
    return json.dumps(filtered_plagiarised_wikipedia_data, indent=None if no_pretty else indent_size)
